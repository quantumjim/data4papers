...
def get_params(backend_name):
    ...
    links = [[] for c in range(3)]
    for plaquette in plaquettes:
        for c in range(3):
            links[c] += plaquette[c]
    for c, color_links in enumerate(links):
        links[c] = list(set(color_links))
        
    return N, plaquettes, colors, links, processor
...
def get_circuit(rounds, backend_name, N, plaquettes, colors, links, dd):
...
def run_experiment(backend_name,rounds,prob=None,num_copies=None,processor=None,dd=True,provider=None):
    ...
    if not processor:
        processor = backend_name
    N, plaquettes, colors, links, processor = get_params(processor)
...
def get_results(archive_id):
    ...
    N, plaquettes, colors, links, _ = get_params(processor)
    ...
    T = 2 # should be something like rounds/4, but hardcoded for now

    link_bits = {}
    a_bits = {}
    for color_links in links:
        for _,v0,a,v1 in color_links:
            a_bits[a] = []
            for r in ['z','x']:
                link_bits[r,v0,a,v1] = []
    ...
    w_av_z = [[[0 for _ in range(num_copies)] for _ in range(T-1)] for p in range(len(plaquettes))]
    w_av_x = [[[0 for _ in range(num_copies)] for _ in range(T-1)] for p in range(len(plaquettes))]
                ...
                for c in range(3):
                    for p in colors[c]:
                        ...
                        for r in ['z', 'x']:
                            w[r] = ''
                            for dc in [1,2]:
                                for _,v0,a,v1 in plaquette[(c+dc)%3]:
                                    for dt in dts[c][r][(c+dc)%3]:
                                        b = link_bits[r,v0,a,v1][t0+dt]
                                        w[r] += string[-b-1]
                                        last_t = a_bits[a].index(b)-1
                                        if last_t>=0:
                                            b = a_bits[a][last_t]
                                            w[r] += string[-b-1]

                        if w['z'].count('1')%2==1:
                            w_av_z[p][t0][copy] += counts[copy][string]
                        if w['x'].count('1')%2==1:
                            w_av_x[p][t0][copy] += counts[copy][string]
                        w_samples[p][t0][copy] += counts[copy][string]

        for t in range(T-1):
            for p in range(len(plaquettes)):
                w_av_z[p][t][copy] /= w_samples[p][t][copy]
                w_av_x[p][t][copy] /= w_samples[p][t][copy]
                
    for t in range(T-1):
        for p in range(len(plaquettes)):
            w_av_z[p][t] = np.mean(w_av_z[p][t])
            w_av_x[p][t] = np.mean(w_av_x[p][t])
                
    return w_av_z, w_av_x